...
//					String iSocietyID = sAryDetails[1];
					int visitorId = Integer.parseInt(sVisitor);
					HashMap DecryptedTokenMap = objDirectory.getDecryptedTokenMap();
					Integer iSocietyID = Integer.parseInt((String) DecryptedTokenMap.get("society_id"));				
					HashMap objHash = objDirectory.mfetchVisitorDetails(visitorId,iSocietyID);			
					JSONObject jsonObject1 = new JSONObject(objHash);

					if (jsonObject1.has("details")) {
					    JSONObject details = jsonObject1.getJSONObject("details");

					    if (details.has("VisitorDetails")) {
					        JSONObject visitorDetails = details.getJSONObject("VisitorDetails");

					        if (visitorDetails.has("visitors")) {
					            JSONArray visitorsArray = visitorDetails.getJSONArray("visitors");

					            if (visitorsArray.length() > 0) {
					                JSONObject firstVisitor = visitorsArray.getJSONObject(0); // Get first visitor
					                
					                String visitorJson = firstVisitor.toString();

					                // Send the response
					                response.setContentType("application/json");
					                out.println("{\"success\":1,\"response\":" + visitorJson + "}");
					                return;
					            }
					        }
					    }
					...
					response.setContentType("application/json");
					out.println("{\"success\":0,\"message\":\"No visitor found\"}");
